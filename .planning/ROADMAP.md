...
- [ ] **Phase 1: Infrastructure** - Install and configure Vitest, Playwright, and the fixture generator so tests can run at all
- [ ] **Phase 2: Parser Unit Tests** - Cover all reference extraction logic in `parser.ts` including edge cases and error handling
- [ ] **Phase 3: Graph Unit Tests** - Cover `buildGraph()`, layout modes, and edge classification in `graph.ts`
