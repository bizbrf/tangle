...
3. [Layer responsibilities](#layer-responsibilities)
   - [Parsing layer (`src/lib/parser.ts`)](#parsing-layer)
   - [Graph construction layer (`src/lib/graph.ts`)](#graph-construction-layer)
   - [Reference resolution layer (`src/lib/resolver.ts`)](#reference-resolution-layer)
   - [Filename sanitization (`src/lib/filenameSanitizer.ts`)](#filename-sanitization)
   - [Shared types (`src/types.ts`)](#shared-types)
   - [React components (`src/components/`)](#react-components)
...
The parser makes **no assumptions about the React layer**. It is a pure `async` function that takes raw bytes and returns typed objects.
...
Push / PR to main
  ...
  └── Playwright E2E (Chromium + Firefox + WebKit)
        ...
        └── Tauri build → Windows installers → GitHub Release
```

**Important:** `ci.yml` must not reference a GitHub Pages environment — doing so causes `action_required` status on PR checks. The Pages deployment is handled exclusively in `deploy.yml`.
